	...
	$.extend($.fn, {
		simpleLightboxVideo : function() {
			...
			video.click(function()	{
				...
				var end = '</div></div><div id="slvj-background-close"></div></div>';

				$('body').append(win + back + lightbox + close + ifr + end);
				$('#slvj-window').hide();

				if ($(this).data('videosite') == "youtube") {
					var url = 'https://www.youtube.com/embed/' + $(this).data('videoid') + '?rel=0&autoplay=1';
